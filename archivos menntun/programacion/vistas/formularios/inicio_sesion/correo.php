...
//Load Composer's autoloader
            ...
            try {
                ...
                $mail->addAddress($correo);     //Add a recipients //Optional name

                //Content
                $mail->isHTML(true);                                  //Set email format to HTML
                $mail->Subject = 'Cambio de contraseña';
                $mail->Body    = 'Espero que estes muy bien. En estos momentos, solo falta un paso para terminar el proceso de cambio de contraseña. <br><a href="http://localhost/menntun/vistas/formularios/inicio_sesion/formulario_can_c.php"><input type="button" style=" color: blue; background: white; border-radius: 5px; border: solid blue 1px; " value="Click aqui"></a>';
                //$mail->AltBody = 'This is the body in plain text for non-HTML mail clients';
                $mail->CharSet = 'UTF-8';
                $mail->send();
                echo "<script>alert('¡Felicidades! Se ha enviado un correo para el cambio de contraseña');window.location.href='login_alumno.php'</script>";
            } catch (Exception $e) {
                echo "Error {$mail->ErrorInfo}";
            }
}else{
    echo "<script>alert('Menntun no encontró el correo del alumno');window.location.href='cambiar_contraseña.php'</script>";
}